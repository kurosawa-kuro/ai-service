# AIサービス個人カタログ 構想ブレスト (生ログ)

> 退避元: docs/01_requirements.md の冒頭に貼られていた未整理ブレスト。
> 確定した仕様は 01_requirements.md、基礎設計は 02_architecture.md に蒸留済み。
> ここは出典・サービス見出し候補・思考過程の素材として保持する。権威順位は archive (最下位)。
