...
`src/21_サービス/`（サービス台帳）／ `src/22_用途/`（用途逆引き）／ `src/30_評価/`（評価ログ）／ `src/23_候補/`（候補）／ `src/12_使い分けルール.md`（使い分け）／ `src/10_契約/01_現在の契約.md`。

## 進め方

- 手書きで気軽に足す個人台帳。重いプロセス（承認ゲート・検証レベル・タスク契約・scope-creep 検出）は持たない。**ハーネスは AI を導くもので、owner を縛らない。**
- `src/21_サービス/<name>.md` は既存ファイルと同じ見出し構成で。料金 / API / CLI / MCP / ローカル可否は確認日つきで、不明は `未確認`。
