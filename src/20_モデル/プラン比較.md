...
> 現在の契約状況は [../10_契約/01_現在の契約.md](../10_契約/01_現在の契約.md) を参照（このファイルは「選択肢」、契約は「実際に払っているもの」）。
...
> 更新規約: 台帳（`21_サービス/README.md`）に従い、料金変更を見たら確認日を差し替える。個別サービスの詳細は各 `21_サービス/<name>.md` の「基本」表へ反映してよい（現状 `未確認` のものが多い）。
