...
| `contracts/` | 契約管理。**番号＝移行経路順**で `01_現在の契約.md`（現状 $310）→ `02_AIフロンティア・バーベル戦略.md`（中間目標 ≈$120・第二理想）→ `03_将来方針.md`（最終目標 $60・第一理想）。番号は優先度ではない |
| `strategies/` | 契約済みキーを**どう使うか**の戦略。`AI-Model API活用戦略.md`（Open WebUI＋LiteLLM ／ reasoning-orchestrator ／ ai-gateway の2軸＋計量） |
| `education/` | 外部教材リポ `ai-tool` との連携。`教材化リンク.md` |
| `usecases/使い分けルール.md` | 使い分け判断の正本 |
| `services/コーディングプラン比較.md` | コーディング系サービスの料金プラン横比較（選択肢） |
...
- 使い分けが固まったら `usecases/使い分けルール.md` に反映する。
...
3. 未登録の候補を探すなら `candidates/カテゴリ別候補棚.md` を見る。
4. 実際に使ったら `evaluations/` にログを作る。
5. 判断が固まったら `usecases/使い分けルール.md` を更新する。
